...
{
	if (mas[i] > 0 && end->finish[i]->pyth[1]->open_room == 0)
	{
...
static int ft_check_push_first_arm(int *mas, int *flag)
{
	...
	if (out_put_name_arm == 0)
	{
		...
		{
			if ((end->finish[i]->col_room - 2) == 0)
			{
				j = 0;
				while (++j <= end->N)
					ft_output_arm(j, end->finish[i]->pyth[1]->name_room);
				*flag = -1;
			}
		}
		i = -1;
		while (*flag != -1 && end->finish[++i] != NULL)
		{
			if (mas[i] > 0)
			{
//				ft_push(i, 1, out_put_name_arm + 1);
	...
	else if (*flag == 1 && end->finish[0]->pyth[1]->open_room == 0)
	{
		*flag = 0;
